...
public:
  ...
  size_t GetColumnCount() const {
    ...
    const int count =
        static_cast<int>((mData.size() > firstRow) ? mData.at(firstRow).size()
                                                   : 0) -
        (mLabelParams.mRowNameIdx + 1);
  ...
  void ReadCsv(std::istream &pStream) {
    ...
    if ((bom2b == bomU16le) || (bom2b == bomU16be)) {
      ...
      const std::wstring &utf16 = [&]() {
        if (mIsLE) {
          const std::codecvt_mode mode = static_cast<std::codecvt_mode>(
              std::consume_header | std::little_endian);
          std::wstring_convert<std::codecvt_utf16<wchar_t, 0x10ffff, mode>>
              utf16conv;
